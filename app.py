...
# --- Finnhub API key input (placeholder, user-provided) ---
...
# Note: no real API key is stored in the file. If left blank, news will be disabled.
...
def get_company_news(symbol, api_key):
    """
    Fetch company news from Finnhub using the provided api_key.
    If no api_key is provided, return an empty list.
    """
    if not api_key:
        # No API key provided — don't attempt network call.
        return []
